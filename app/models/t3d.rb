class T3d < ApplicationRecord
  ...
  def self.t3d_search(term)
    where('LOWER(name) LIKE :term OR LOWER(description) LIKE :term', term: "%#{term.downcase}%")
  end

  # softopen
  has_many :canopens, dependent: :destroy
  has_many :softopen3ds, through: :canopens
  ...
  has_many :dataositems, dependent: :destroy
  has_many :ositems, through: :dataositems
